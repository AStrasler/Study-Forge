...
class Settings:
    ...
    provider_fallback_order: List[str] = field(
        default_factory=lambda: ["ollama", "fox", "mullama", "groq"]
    )
    ...
    def load(cls, env_file: str | Path | None = None) -> "Settings":
        ...
        return cls(
            ...
            fox_model=os.getenv("FOX_MODEL", os.getenv("LOCAL_MODEL", "llama3.2")).strip(),
            fox_timeout=int(os.getenv("FOX_TIMEOUT", "300")),
            mullama_base_url=os.getenv("MULLAMA_BASE_URL", "http://localhost:11435").rstrip("/"),
            mullama_model=os.getenv("MULLAMA_MODEL", os.getenv("LOCAL_MODEL", "llama3.2")).strip(),
